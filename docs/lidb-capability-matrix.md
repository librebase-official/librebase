...
| 2 | REST `/rest/v1` | PostgREST | **lis** `routes/rest` + `packages/lis-rest` | 🚧 | Python MVP CRUD `parity_items` ✅. Wave 4: `.li` allowlist builds under self-host lic @ `08ecd3e`; `LI_REST_REQUIRE_LI=1` fail-closed. Li HTTP serve blocked on lic P0 httpd |
