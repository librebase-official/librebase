...
| 6 | Object Storage | Storage | **lis** `routes/storage` | ✅ | Buckets + HMAC + **SigV4 query GET** (host-bound) + TUS stub. `P-STO-03` parity: signed GET round-trip + anonymous deny. **CDN resize `oos_lean`** — `/render/image` passthrough only |
